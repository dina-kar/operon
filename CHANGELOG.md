...
- Collection GC roots and trimming (M1.1): `CollectionGcRoots` (Lance reachability, splits, bitmaps, manifests, PK deltas and dead letters of the retained chain), `PkGcRoots`, GC of retired prefixes after the grace period, and `CollectionTrimSource`, which trims implicit streams below the oldest retained manifest. The server runs collection link apply, index builds, trimming and collection GC.
...
- Collection coverage in the M0 gates (M1.1): crash-gate rows for every collection commit failpoint (`collection.after_lance_commit`, `.after_split_put`, `.after_manifest_put`, `.after_cas`, `.after_pk_write`) and both index-build failpoints (`collection.index.after_lance_commit`, `.after_cas`); the fault matrix's `CollectionCommit` row; and a collection in the `operon-sim` simulation with its model check.
...
- `operon-store` (M1.1): `StoreError::is_retryable` (true for `Backend`, `AlreadyExists` and `PreconditionFailed`).
