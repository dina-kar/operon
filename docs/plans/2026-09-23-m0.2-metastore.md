...
| 8 | `MetaNode::start` does not wait for recovery (openraft's `wait_for_recovery` needs a quorum) | A node must be able to restart while its peers are down. openraft re-applies committed entries in the background, because the log store persists the commit index | `Consistency::Local` reads can briefly return an older state after a restart. That is documented on `Consistency::Local` and in the Task 6 code |
...
- **Install.** Verify the snapshot, persist it the same way, then swap the state in. A corrupt snapshot, or one whose embedded metadata differs from openraft's, fails with `InvalidData` and changes nothing.
...
- `on_leader_among` retries only `NotLeader`, which means the command was never proposed. It never retries `Timeout`, whose outcome is unknown.
...
Run the cluster and node tests 20 times while the full workspace test suite competes for the CPU:

```bash
for i in $(seq 1 20); do
  (cargo test -q --workspace >/dev/null 2>&1 &)
  cargo test -q -p operon-meta --test cluster --test node || break
  wait
done
```

Expected: 20 clean runs. A failure here is a real race: find its cause, and do not add sleeps or retries of `Timeout`.
