...
| Arrow Flight SQL with `DoPut` ingest (D49) | M1.2 (Task 13), M1.6 (Python `ingest`), M1.7 (ADBC gate) |
...
| The ADBC Flight SQL drivers (Python and Go) pass their conformance tests against Operon, `DoPut` bulk ingest included (D49) | M1.7 (surface built in M1.2) |
