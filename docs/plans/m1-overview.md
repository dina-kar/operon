...
| [M1.2a: `MetaStore` trait](2026-09-25-m1.2a-metastore-trait.md) | `m1.2a-metastore-trait` | `trait MetaStore` in `operon_common::meta` (D47): the shared metastore types move from `operon-meta` (re-exported, on-disk encoding unchanged); the openraft `MetaClient` is the first implementation; `operon-worker`, `operon-log`, `operon-link`, `operon-collection` and the `operon` API layer take `Arc<dyn MetaStore>`; `operon-meta-conformance` (conformance suite with linearizability checks, run on the openraft single node and 3-node cluster); M0 and M1.1 gates re-run unchanged | M1.1 |
...
| [M1.5: Elasticsearch API Phase A](2026-09-24-m1.5-elasticsearch-api.md) | `m1.5-elasticsearch-api` | `operon-es`: REST gateway (9200) per §06 §7 Phase A as D48 defines it: document APIs, `_bulk`, `_search`/`_count`/`_msearch` with the core DSL, `knn`, hybrid + RRF, the fixed `script_score` vector scripts, `search_after`, `_delete_by_query`, minimal index admin (A34); multi-target aliases with a write index in the catalog and `CollectionService` (D57, A41) | M1.2 |
| [M1.6: SDKs and MCP server](2026-09-24-m1.6-sdks-mcp.md) | `m1.6-sdks-mcp` | Python and TypeScript SDKs for the native API (plus Flight SQL, `to_arrow()`/`to_polars()` and `scan_plan()` from Python; A40); `operon-mcp`, the W0 MCP server on the 2026-07-28 stateless spec | M1.2 |
...
| The ADBC Flight SQL drivers (Python and Go) and Spice's Flight SQL connector pass against Operon, `DoPut` bulk ingest included (D49, D56) | M1.7 (surface built in M1.2) |
...
| `operon-meta-conformance` (M1.2a, `publish = false`) | The backend-agnostic `MetaStore` conformance suite (`metastore_conformance!`), its `Backend`/`Faults` harness, and the linearizability checker (moved from `operon-sim`, which re-exports it) | common |
...
**Added 2026-09-26 (turbopuffer gap analysis).** M1.3 Task 15 answers collection writes over the unapplied budget with 429 (`resource_exhausted`, `Retry-After`) and adds the response headers `Operon-Unapplied-Records` and `Operon-Unapplied-Bytes` and the request header `Operon-Backpressure: off` (A45, D86). M1.5 Task 9a adds `POST …/collections/{c}/documents/delete_by_filter` and `…/documents/patch_by_filter` (A46, D87). M1.6 Task 10 adds `performance` and `Server-Timing` to search and SQL responses; M1.7 Task 10 adds `POST …/collections/{c}/recall` (A47, D92). M1.6 Task 12 publishes the native REST API as `docs/api/openapi.json` (D101).
...
- Only the composition roots (`operon`, `operon-sim`) depend on `operon-meta` outside dev-dependencies; every other crate reaches the metastore through `Arc<dyn MetaStore>` (D47, M1.2a). A new metastore need is added as a named, semantic trait method with conformance cases, never as a closure over `MetaState` or a raw `Command`. Gateways hold no metastore handle (A33).
...
| A35 | §2, §3 (D49): the ADBC Flight SQL drivers (Python and Go) pass their conformance tests against Operon, `DoPut` bulk ingest into a collection and a stream included. M1.7 Task 4 runs `adbc-go` (ADBC's `validation` suites v1.12.0 with an Operon `DriverQuirks`) and `adbc-python` (DBAPI checks over `adbc-driver-flightsql` 1.12.0); neither has an oracle (M1.7 Ruling 16). The elasticsearch-py client suite is not run (D48). M1.7 depends on M1.2 Tasks 12–13 for the Flight SQL surface | Owner decision 2026-09-25 (D49) |
