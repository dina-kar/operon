...
- **Metastore-referenced objects under the relaxed contract** (M2, D59): before GC deletes an object the metastore references (a WAL object, a segment, a manifest behind a pointer), it writes a claim for the object's path, and every command that makes an object reachable fails if a claim exists. Objects reachable only through a manifest stay protected by the freshness check and the grace period, whose margin includes `max_clock_skew`. GC reads tags before retention, a documented read order, instead of one snapshot (§18 §3).
