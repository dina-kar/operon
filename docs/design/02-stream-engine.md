...
- Segments keep the WAL's batch bytes verbatim ⇒ segmenting never re-encodes, and fetch decodes straight from the cached byte range.
...
4. `arrow` encoding: Arrow IPC per chunk vs. one Arrow file per segment with a column index; and whether the WAL writes `arrow` directly or the segmenter converts.
