...
| Event ingest, agent traces (Kafka) | Stream | Operon log segments on S3 | Native streaming API over HTTP and gRPC (idempotent produce, streaming subscribe, named consumers) and Flight `DoPut`/`DoGet` (D43) | M5 |
