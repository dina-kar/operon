...
| *Proposed* | Agent workspaces (§15) | W0 (with M1): MCP server. W1 (after M2): Git on the bucket with O(1) forks, code-index links, credential vending, agent telemetry. W2: `operon-sandbox` with copy-on-write environment images, package caches, registry proxy. W3 (Phase C): `jj` backend, REAPI, VM snapshots | Per phase in §15 §13 |
...
| 17 | Scope: agent workspaces (§15) add a Git server, a filesystem layer and a registry proxy | High | High | Keep it proposed until M1 ships; W0 (MCP) alone first; buy nydus, gitoxide and runtimes; no runtime, POSIX FS or GitHub UI of our own |
