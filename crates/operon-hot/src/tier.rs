...
use operon_collection::{
    CollectionContext, CollectionManifest, CollectionSnapshot, HotArtifactRef, vector_column,
};
use operon_common::meta::{ApplyError, Consistency, Lease, MetaError};
...
pub struct ReconcileReport {
    ...
    pub dropped: u32,
    /// One line per failed collection or artifact load.
...
struct TierInner {
    ...
    state: RwLock<HashMap<(NamespaceId, CollectionId), Columns>>,
    counters: Counters,
...
impl HotTierImpl {
    ...
    /// drive the tier themselves; row 6.5). Removes `dir/hnsw/` and
    /// `dir/delta/` first (rule 6): nothing local is trusted across restarts.
    pub async fn new(
        ctx: CollectionContext,
        config: HotTierConfig,
        node_id: u64,
    ...
    fn read_state(
        &self,
    ) -> std::sync::RwLockReadGuard<'_, HashMap<(NamespaceId, CollectionId), Columns>> {
        self.inner
            .state
            .read()
    ...
    /// Rule 5: one pass over every collection. A failure of one collection
    /// or artifact is logged and reported, and retried on the next pass.
    pub async fn reconcile_once(&self) -> Result<ReconcileReport, TierError> {
        ...
        for head in heads {
            ...
            if head.pointer.is_none() || collection.schema.vectors.is_empty() {
                continue;
            }
            ...
            if !effective_hot(pinned, lease, inner.config.pin_all, now_ms).vectors {
                continue;
            }
            wanted.insert((ns, cid));

            // 2. Load, extend and publish.
            match self.reconcile_collection(ns, cid, &mut report).await {
                Ok(columns) => {
                ...
                Err(err) => {
                    tracing::warn!(namespace = %ns, collection = %cid, %err, "reconciling a hot collection failed");
                    report.failures.push(format!("{ns}/{cid}: {err}"));
                }
            }
        }

        // 3. Collections no longer owned, hot or present.
        let mut state = inner.state.write().unwrap_or_else(PoisonError::into_inner);
    ...
    /// Rule 5.2 for one owned, hot collection: its new column states.
    async fn reconcile_collection(
        ...
        let inner = &*self.inner;
        let ctx = &inner.ctx;
        let snapshot = CollectionSnapshot::open(ctx, ns, cid, Consistency::Local).await?;
        let old = self
            .read_state()
            .get(&(ns, cid))
            .cloned()
